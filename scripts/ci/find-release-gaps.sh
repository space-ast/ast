...
#
# 状态码走 stdout 是为了能直接写进 $()；响应体落文件，出错时还能原样截一段进日志。
...
# 列出 Gitee 上已有的附件名（每行一个，已排序）。
#
# Gitee 的 release 对象里只有自动生成的源码包，上传的附件要另外查 attach_files。
# release 还不存在时输出空列表 ——「一个都没有」正好等于「全部待补发」
gitee_asset_names() {
    local tag="$1" code id
    ...
    case "$code" in
        ...
        404)
            log "Gitee 上还没有 $tag 的 release，全部按待补发处理"
            return 0
            ;;
        *)
    ...
    # 一个附件都没有时 Gitee 返回的是 null 而不是 []，按空列表处理（action 里也是这么兜的）。
    # 直接 .[] 会报 "Cannot iterate over null"，把整个 plan 带崩。
    jq_lf -r '(. // [])[].name' "$WORK/attach.json" | LC_ALL=C sort
}

# 列出 GitCode 上已有的附件名（每行一个，已排序）。
#
# GitCode 的 release 对象里 assets 混了两类东西：自动生成的源码包（type: source，
# 没有数字 id）和上传的附件（type: attach，带 id）。先按 id 筛，再用 URL 排掉源码包，
# 这两条判据与 action 里的筛选保持一致 —— 少一条就可能把源码包当成「这个附件已经有了」，
# 于是漏补。
gitcode_asset_names() {
    local tag="$1" code
    ...
    case "$code" in
        200) ;;
        404)
            log "GitCode 上还没有 $tag 的 release，全部按待补发处理"
            return 0
            ;;
        *)
...
# 镜像仓库上有没有这个 tag。
#
# 建 release 时 tag 不在，平台会顺手把 tag 建出来：GitCode 那边被镜像规则拒掉，
# Gitee 那边则会在镜像之外造出一个分叉 tag。所以两个平台都要先等 tag 同步到位。
#
# 刻意区分「还没有」和「查不到」：前者是正常的等待，后者是网络问题，不能悄悄当成
# 「没有」——那样这一轮会直接跳过，且看不出任何异常。
remote_has_tag() {
...
main() {
    ...
    # 凭据也一律不检查：gh 自己会先读 GH_TOKEN / GITHUB_TOKEN，读不到再退回本机
    # `gh auth login` 的登录态；真没凭据时 gh 会自己报错，脚本照样退出非零。
    #
    ...
    for p in gitee gitcode; do
        ...
        # 两个平台都得等镜像把 tag 同步过来，否则建 release 会造出分叉 tag / 被镜像规则拒。
    ...
    # 空列表序列化成 []，好让 workflow 那边的 if 把整个 job 挡掉。
