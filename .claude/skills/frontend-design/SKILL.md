...
## Qt/C++ Desktop Adaptation

When applying these principles to Qt/C++ desktop applications:

1. **QSS (Qt Style Sheets)** is the primary styling mechanism — treat it like CSS
2. Use `QPalette` for system-level color theming where QSS falls short
3. Leverage `QGraphicsDropShadowEffect`, `QGraphicsOpacityEffect` for depth
4. Custom `QStyle` subclasses for advanced rendering control
5. `QPropertyAnimation` for motion and transitions
6. Match the aesthetic direction to the domain: scientific tools → refined technical, creative tools → bold expressive
