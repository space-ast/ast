...
errc_t SweepVariable::getValue(double& value) const
{
    if (!expr_)
...
errc_t SweepVariable::setValue(double value)
{
    if (!expr_)
        return eErrorNullPtr;
...
errc_t SweepOutput::getValue(double& value) const
{
    if (!expr_)
        return eErrorNullPtr;
    return expr_->getValueDouble(value);
...
void SweepStudy::addVariable(SweepVariable* variable)
{
    if (variable)
        variables_.push_back(variable);
}

void SweepStudy::removeVariable(SweepVariable* variable)
{
...
void SweepStudy::addOutput(SweepOutput* output)
{
    if (output)
        outputs_.push_back(output);
}

void SweepStudy::removeOutput(SweepOutput* output)
{
...
errc_t SweepStudy::executeStep(int stepIndex)
{
    ...
    for (auto& output : outputs_)
    {
        if (!output)
            continue;
        double value = 0.0;
        errc_t ret = output->getValue(value);
        if (ret == eNoError)
        {
