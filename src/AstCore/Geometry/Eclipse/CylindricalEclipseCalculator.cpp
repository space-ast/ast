///
/// @file      CylindricalEclipseCalculator.cpp
/// @brief     
/// @details   
/// @author    axel
/// @date      2026-06-10
...
double CylindricalEclipseCalculator::getLightingRatio(const TimePoint& time, const Vector3d& position, Frame* frame)
{
    // 无光源或无遮挡体，视为全光照
    ...
    for (auto& occultingBody : occultingBodies_)
    {
        ...
        double ratio = aLightingRatio_CylindricalModel(obsRelativeToOcc, lightRelativeToOcc, occultingBody->getRadius());
        if (ratio < minRatio)
        {
            minRatio = ratio;
        }
