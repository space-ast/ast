...
public:
    ...
    /// @brief 是否为空区间（start_ > stop_，弱序否定；反向 / {+∞,-∞} 哨兵 / NaN → true）
...
public:
    ...
    ///          点区间（start == stop）返回 1，否则返回 0（无有效采样点）。该数量不依赖于基准 epoch。
    /// @param step 离散化步长（秒）
    /// @return 采样点数
...
public:
    ...
    /// @brief 判断两个时间区间是否相交（非空交集，含仅相切产生的点区间）
    /// @param other 另一个时间区间
    /// @return 是否相交
    AST_CORE_API
