///
/// @file      AndConstraint.hpp
/// @brief     与约束组合器
/// @details   min(所有子约束值)，全部满足才算满足，短路计算。
/// @author    axel
/// @date      2026-07-22
...
/// @brief 与约束组合器
/// @details 所有子约束同时满足才算满足。
///          evaluate() 返回 min(子约束值)，遇到非正立即短路。
class AST_CORE_API AndConstraint : public AccessConstraint
{
public:
    AST_OBJECT(AndConstraint)
