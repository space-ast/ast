...
//                            (in jr_drag header file)
   ...
   #if 0
   // 代码审查：当向量共线时，下面的代码会执行除以 0 操作，导致运行错误
   ...
   #else
   double cross = sun[0] * space_craft[1] - sun[1] * space_craft[0];
   double dot   = sun[0] * space_craft[0] + sun[1] * space_craft[1];
   double hour_angle = atan2(cross, dot);
   #endif
