...
QString aUiObjectDisplayName(Object* obj)
{
    if(!obj)
        return QObject::tr("<空>");
    std::string displayName = obj->displayName();
    return displayName.empty() ? QObject::tr("未命名<%1>").arg(QString::fromStdString(obj->typeName())) : QString::fromStdString(displayName);
}
