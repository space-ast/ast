///
/// @file      UiStudyWorkbench.cpp
/// @brief     StudyWorkbench 编辑界面实现
...
UiStudyWorkbench::UiStudyWorkbench(QWidget* parent)
    : QWidget(parent)
{
    setupUi();
}

void UiStudyWorkbench::setupUi()
{
...
void UiStudyWorkbench::setStudyWorkbench(StudyWorkbench* analyzer)
{
    if (studyWorkbench() == analyzer)
        return;

    analyzer_ = analyzer;

    auto* a = studyWorkbench();
    if (a)
    {
...
void UiStudyWorkbench::refreshUi()
{
    auto* a = studyWorkbench();
    if (a)
    {
...
void UiStudyWorkbench::rebuildCommandEditor()
{
    // 移除旧编辑器（index 1，如果存在）
